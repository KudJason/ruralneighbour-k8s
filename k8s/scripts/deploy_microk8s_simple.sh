#!/bin/bash
# ms-backend/k8s/deploy_microk8s_simple.sh
# 简化的 MicroK8s 部署脚本
...
echo -e "${BLUE}🚀 简化 MicroK8s 部署到 home.worthwolf.top${NC}"
echo "=============================================="
...
for service in auth-service user-service location-service request-service payment-service notification-service content-service safety-service; do
    echo "等待 $service 启动..."
    microk8s kubectl wait --for=condition=ready pod -l app=$service --timeout=300s -n ruralneighbour || echo "警告: $service 启动超时"
done
